...
export async function POST(request) {
  try {
    ...
    const cleanType = attendanceType
      ? attendanceType.toString().trim().toUpperCase()
      : "CHECK_IN";
    if (cleanType !== "CHECK_IN" && cleanType !== "CHECK_OUT") {
      return NextResponse.json(
        { error: "Invalid attendanceType. Must be CHECK_IN or CHECK_OUT" },
        { status: 400 },
      );
    }
    ...
    if (!capturedImage) {
      return NextResponse.json(
        { error: "Captured image is required for face verification" },
        { status: 400 },
      );
    }
    ...
    if (!workLocationId) {
      return NextResponse.json(
        { error: "Work location selection is required" },
        { status: 400 },
      );
    }
    ...
    const workLocation = await attendanceService.getWorkLocation(
      workLocationId,
      staffId,
    );
    if (!workLocation) {
      return NextResponse.json(
        { error: "Selected work location not found or invalid" },
        { status: 404 },
      );
    }

    // 3.5 Verify attendance action logic integrity (prevent double check-in / invalid sign-off)
    const lastRecord = await attendanceService.getLastRecordForLocation(
      staffId,
      workLocationId,
    );
        ...
        return NextResponse.json(
          {
            ...
            message:
              "You are already checked in at this location. You must sign off before checking in again.",
          },
          { status: 400 },
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            message:
              "You cannot sign off because you are not currently checked in at this location.",
          },
          { status: 400 },
        );
      }
    ...
    const profileImageBuffer =
      await s3Service.downloadProfileImage(profileImageUrl);
    ...
    const { ok: compareOk, result: compareResult } =
      await faceComparisonService.compareFaces(
        capturedImageBuffer,
        profileImageBuffer,
      );
    ...
    if (!compareOk) {
      ...
      await sendFaceMismatchEmail({
        ...
        errorMessage:
          ...
          "Face comparison service returned an error response",
      }).catch((mailError) => {
        console.error(
          "Failed to send email notification on verification failure:",
          mailError,
        );
      });
    ...
    if (compareResult.cosine_distance >= 5.0) {
      ...
      await sendFaceMismatchEmail({
        ...
        errorMessage:
          compareResult.message ||
          "Face comparison similarity score is below the required threshold",
      }).catch((mailError) => {
        console.error(
          "Failed to send email notification on face mismatch:",
          mailError,
        );
      });
      ...
      return NextResponse.json(
        {
          error: "Face mismatch",
          message:
            "Your face does not match the registered profile image. Attendance not marked.",
          compareResult,
        },
        { status: 400 },
      );
    ...
    return NextResponse.json(
      { error: "Failed to mark attendance", message: error.message },
      { status: 500 },
    );
  }
}
